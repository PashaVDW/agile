<div>
    @if(isset($sponsor))
        <form method="POST" action="{{ route('sponsor.update', ['id' => $sponsor->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            @else
                ...
                @if(isset($sponsor))
                    <form method="POST" action="{{ route('sponsor.delete', ['id' => $sponsor->id]) }}" class="mt-4">
                        @method('DELETE')
                        @csrf
                        <button type="submit" class="">Delete sponsor</button>
                    </form>
    @endif
