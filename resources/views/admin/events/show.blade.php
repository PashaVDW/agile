...
<div class="container">
    <form
        method="POST"
        action="{{ isset($event) ? route('admin.event.update', ['id' => $event->id]) : route('admin.event.store') }}"
    ...
    >
        ...
        />
        <x-forms.input-textarea name="description" label="Beschrijving" :class="'max-h-[300px]'">
        ...
        <x-forms.input-field
            ...
            label="Datum"
            :required="true"
            value="{{ old('date',$event->formatted_date_for_input ?? '' )}}"
        />
        ...
        <x-forms.input-select
            ...
            label="Categorie"
            :enum="$categories"
            value="{{old('category', $event->category->value ?? '')}}"
        />
        <x-forms.input-field
            name="payment_link"
            label="Betaal link"
        ...
        @if ($sponsors->count() > 0)
            ...
            @foreach ($sponsors as $sponsor)
                <x-forms.input-checkbox
                    ...
                    :value="$sponsor->id"
                    :label="$sponsor->name"
                    :checked="isset($event) && $event->sponsors->contains($sponsor->id)"
                />
            @endforeach
        @endif

        @if (! isset($event) || $event->status->name !== 'ARCHIVED')
            <button id="openModalButton" type="button" class="button right hidden">
                {{ isset($event) ? 'Evenement updaten' : 'Evenement toevoegen' }}
            </button>
        @endif

        <button id="submitButton" type="submit" class="button right">
            {{ isset($event) ? 'Evenement updaten' : 'Evenement toevoegen' }}
        </button>
