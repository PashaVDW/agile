    ...
    <div class="filter-wrapper">
        <form method="GET" action="{{ route(Route::currentRouteName()) }}">
            <x-forms.input-select
                ...
                name="status"
                enum="{{\App\Enums\ActiveTypeEnum::class}}"
            ...
            @foreach ($events as $event)
                <tr>
                    <td>
                        ...
                        <span>
                            {{ $event->status->name === 'ARCHIVED' ? '(' . __('ARCHIVED') . ')' : '' }}
                        </span>
                    </td>
                    <td>{{ $event->formatted_date }}</td>
                    <td>{{ __($event->category->value) }}</td>
                    <td>
                        {{ $event->getFormattedDateTime($event->created_at) }}
                    </td>
                    <td>
                        {{ $event->getFormattedDateTime($event->updated_at) }}
                    </td>
                    <td>
                        <a href="{{ route('admin.event.show', ['id' => $event->id]) }}">Updaten</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
